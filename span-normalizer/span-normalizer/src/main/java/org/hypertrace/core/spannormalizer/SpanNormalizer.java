...
import static org.hypertrace.core.spannormalizer.constants.SpanNormalizerConstants.BYPASSED_OUTPUT_TOPIC_CONFIG_KEY;
...
import org.hypertrace.core.spannormalizer.jaeger.ByPassPredicate;
...
import org.hypertrace.core.spannormalizer.jaeger.RawSpanToStructuredTraceTransformer;
...
public class SpanNormalizer extends KafkaStreamsApp {
  ...
  public StreamsBuilder buildTopology(
    ...
    String bypassedOutputTopic = jobConfig.getString(BYPASSED_OUTPUT_TOPIC_CONFIG_KEY);
    ...
    ByPassPredicate byPassPredicate = new ByPassPredicate(jobConfig);
    ...
    KStream<TraceIdentity, RawSpan>[] branches =
        preProcessedStream.transform(JaegerSpanToAvroRawSpanTransformer::new)
            .branch(
                (key, value) -> byPassPredicate.test(value),
                (key, value) -> true
            );
